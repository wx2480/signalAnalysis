...
class DailyReport:
        ...
        for i in tqdm(timeline):
            ...
            for j in range(n):
                ...
                if np.isnan(np.mean(_ret_i[lay])):
                    locals()['group_{}'.format(j)].append(0)
                else:
                    locals()['group_{}'.format(j)].append(np.mean(_ret_i[lay]))
            group.append(np.mean(_ret_i))
        print('kouchou')
        
        group = ((np.array(group) + 1) * 0.997).cumprod()
        fig,ax = plt.subplots()
        for i in range(n):
            ax.plot(((np.array(locals()['group_{}'.format(i)]) + 1) * 0.997).cumprod() - group, label = str(i))
        plt.legend()
        np.save('a.npy', np.array(locals()['group_2']))
        plt.show(fig)
    ...
    def ic_test(self):
        ...
        for i in tqdm(timeline):
            ...
            if _ic:
                _ic.append(_ic[-1] + ybottom)
            else:
                _ic.append(ybottom)
        fig,ax = plt.subplots()
        timeline = list(map(str, timeline))
        ax.plot(timeline, ic)
        ax.plot(timeline, _ic)
        plt.show(fig)
